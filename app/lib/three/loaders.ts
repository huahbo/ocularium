...
export class AnatomyAssetManager {
  ...
  constructor(renderer: THREE.WebGLRenderer) {
    ...
    // Draco keeps the eye-anatomy GLB at 1.45 MB (from 26 MB) while decoding
    // straight back to float32 positions — critical because the viewer's
    // Loop-subdivision pass hangs on KHR_mesh_quantization's unit-cube data.
  ...
  private resetMaterials(organ: LoadedOrgan) {
    ...
    organ.meshes.forEach((mesh) => {
      // The procedural anatomy pass re-runs on the next setOrgan, so clear its
      // guard here — otherwise a cache hit would leave the flat GLB colours.
      mesh.userData.anatomyTextured = false;
